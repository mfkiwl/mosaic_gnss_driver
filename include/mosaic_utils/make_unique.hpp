...
#define _MAKE_UNIQUE(TEMPLATE_LIST, PADDING_LIST, LIST, COMMA, X1, X2, X3, X4)          \
    ...
    inline typename _Unique_if<_Ty>::_Single_object make_unique(LIST(_TYPE_REFREF_ARG)) \
    {                                                                                   \
...
#undef _MAKE_UNIQUE
    ...
    typename _Unique_if<_Ty>::_Single_object make_unique(Args &&... args)
    {
    ...
    template <class _Ty>
    typename _Unique_if<_Ty>::_Unknown_bound make_unique(size_t n)
    {
